...
//    FogOfWar:array of array of Boolean;
   ...
   //  messt:real;
    ...
    /// УРОВЕНЬ, Очки
     LevelMission,LevelMissionTip,LMMax:Byte;
...
begin
    ...
    Begin
      ...
      Canvas.Font.Size:=24;
      Canvas.TextOut(Mainform.width div 2 - Canvas.TextWidth('[BETA v. 0.92]') div 2,
                     Mainform.height div 2-200, '[BETA v. 0.92]');
 ...
 begin
  // Низ
   ...
   if (ymax>0) then
     ...
     Begin
        ymin:=trunc((((mapsizey+1)*100-1000-Engine.WorldY)*Engine.WorldScaleY));
...
Begin
   ...
   Begin
    ...
    if (leveltime>=300)and(health<=0)and(fade1<255) then
    Begin
    ...
    if (leveltime>=300)and(health>0) then
      begin    // leveltime:=300;
        //levelmission:=0;
        health:=-100;
        BoomTime:=100;
  ...
  With MyCanvas do Begin
    ...
    Begin
      ...
      DrawStretch(HudImages.Image['mishud'],0,
            ...
            trunc((5+shieldshow-56)*ResolutionScaleX),
      ...
      dop4:=trunc(Shieldtime/10);
       ...
       DrawStretch(HudImages.Image['shieldfon'],0,
            ...
            trunc((370)*ResolutionScaleY2),
            trunc((6-256+128+shieldshow)*ResolutionScaleX),
      ...
      Fonts[1].TextOut(st,
      (160-256+shieldshow)*ResolutionScaleX-__w/2,
    ...
    Begin
      ...
      DrawStretch(HudImages.Image['mishud'],0,
            ...
            trunc((5+levelmissionshow-56)*ResolutionScaleX),
      ...
      begin
       ...
       dop4:=trunc((300-Leveltime)/30);

       DrawStretch(HudImages.Image['shieldfon'],0,
             trunc((6-256+levelmissionshow)*ResolutionScaleX),
            trunc((250+misshift)*ResolutionScaleY2),
            trunc((6-256+128+levelmissionshow)*ResolutionScaleX),
        ...
        DrawStretch(HudImages.Image['shield_'+inttostr(dop4+2)],0,
            ...
            trunc((misshift+250)*ResolutionScaleY2+(128)*ResolutionScaleY),false,false,
            cRGB4(redw[1],greenw[1],bluew[1],
                  trunc(levelmissionshow*(((300-leveltime)-dop4*30))/30)),fxBlend);
      ...
      dop4:=0;
      if levelmissiontip=1 then
        st:=inttostr(lmmax-levelmission)+'/5'//+inttostr(lmmax)
          else
            if levelmissiontip=2 then
            begin
              Fonts[1].Scale:=ResolutionScaleY*0.6;
              ...
              Begin
                st:='';
                dop4:=trunc((300-leveltime)/60);
                st:= inttostr(dop4)+':';

                dop4:=trunc((300-leveltime)-60*trunc((300-leveltime)/60));
                if dop4<10 then
                   st:= st+'0';
                st:= st+inttostr(dop4);
              End
