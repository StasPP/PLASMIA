...
type
     ...
     TSpriteEngine = class
     private
          FSpriteList, FDrawList: TList;
          FDeadList: TList;
          FWorldX, FWorldY,FWorldScaleX, FWorldScaleY : Double;
     ...
     public
          ...
          property WorldScaleX: Double read FWorldScaleX write FWorldScaleX;
          property WorldScaleY: Double read FWorldScaleY write FWorldScaleY;
     ...
     public
          ...
          property FWi: integer read FW write FW;
          property FHe: integer read FH write FH;
...
begin
     ...
     while FDrawList.Count > 0 do
          TSprite(FDrawList.Items[FDrawList.Count - 1]).Free;
     FDrawList.Free;
     FSpriteList.Free;
     FDeadList.Free;
     inherited Destroy;
...
begin
     for i := 0 to FDrawList.Count - 1 do
           TSprite(FDrawList.Items[i]).Draw;
end;
...
begin
     ...
     begin
          I := (L + H) div 2;
          Dif := TSprite(FDrawList.Items[I]).FZ - Sprite.FZ;
          if (Dif < 0) then
               L := I + 1
          else
               H := I - 1;
...
begin
 {ÓÂØÅÙÊ ÂÔÅÔ}
    ...
    if FDrawMode<5 then
    Begin
     if (FX +ImageWidth*ScaleX> FEngine.WorldX + FEngine.VisibleArea.Left)   and
        (FY +ImageHeight*ScaleY> FEngine.WorldY + FEngine.FVisibleArea.Top)   and
        (FX{-ImageWidth*ScaleX }< FEngine.WorldX + FEngine.FVisibleArea.Right) and
        (FY{-ImageHeight*ScaleY}< FEngine.WorldY+ FEngine.FVisibleArea.Bottom) then
     ...
     begin
          ...
          case FDrawMode of
               ...
               0: FEngine.Canvas.DrawColor1(FEngine.Image.Image[ImgName],
                         FPatternIndex,
                         {round}((FX + FOffsetX - FEngine.FWorldX)*FWorldScaleX),
                         {round}((FY + FOffsetY - FEngine.FWorldY)*FWorldScaleY) ,
               ...
               1: FEngine.Canvas.DrawRotateC(FEngine.Image.Image[ImgName],
                         FPatternIndex,
                         {round}((FX + FWorldX + FOffsetX - FEngine.FWorldX)*FWorldScaleX),
                         {round}((FY + FWorldY + FOffsetY - FEngine.FWorldY)*FWorldScaleY),
                         FAngle,
                         FScaleX*FWorldScaleX*sqr(Cos(FAngle))+FScaleY*FWorldScaleY*sqr(Sin(FAngle)),
                         FScaleY*FWorldScaleY*sqr(Cos(FAngle))+FScaleX*FWorldScaleX*sqr(Sin(FAngle)),
                         FMirrorX, FMirrorY,
               ...
               2: FEngine.Canvas.DrawColor4({FEngine.Image.Image[ImgName],
                         FPatternIndex,
                         round((FX + FOffsetX + FWorldX - FEngine.FWorldX)*FWorldScaleX),
                         round((FY + FOffsetY + FWorldY - FEngine.FWorldY)*FWorldScaleY) ,
                         FScaleX*FWorldScaleX, FScaleY*FWorldScaleY, FDoCenter,
                         FMirrorX, FMirrorY, }
    ...
    Begin
       ...
       if ((FX > FEngine.WorldX + FEngine.VisibleArea.Left)   and
        ...
        (FY< FEngine.WorldY+ FEngine.FVisibleArea.Bottom))
        or
         (((FX+FW > FEngine.WorldX + FEngine.VisibleArea.Left)   and
         (FX+FW < FEngine.WorldX + FEngine.FVisibleArea.Right) ) and
        ((FY+FH > FEngine.WorldY + FEngine.FVisibleArea.Top)   and
        (FY+FH < FEngine.WorldY+ FEngine.FVisibleArea.Bottom)) )
        or
         (((FX+FW/2 > FEngine.WorldX + FEngine.VisibleArea.Left)   and
         (FX+FW/2< FEngine.WorldX + FEngine.FVisibleArea.Right) ) and
        ((FY+FH/2 > FEngine.WorldY + FEngine.FVisibleArea.Top)   and
        (FY+FH/2< FEngine.WorldY+ FEngine.FVisibleArea.Bottom)) )
        or
         (((FX < FEngine.WorldX + FEngine.VisibleArea.Left)   and
         (FX > FEngine.WorldX + FEngine.FVisibleArea.Right) ) or
        ((FY+FH < FEngine.WorldY + FEngine.FVisibleArea.Top)   and
        (FY+FH > FEngine.WorldY+ FEngine.FVisibleArea.Bottom)) )
        ...
        Begin
         ...
         if not FVisible then Exit;
          ...
          FEngine.Canvas.DrawColor1(FEngine.Image.Image[ImgName],
                         FPatternIndex,
                         {round}((FX + FOffsetX - FEngine.FWorldX)*FWorldScaleX),
                         {round}((FY + FOffsetY - FEngine.FWorldY)*FWorldScaleY) ,
                         FScaleX*FWorldScaleX, FScaleY*FWorldScaleY, FDoCenter,
...
begin
     ...
     FVelocityX := FVelocityX + FAccelX * UpdateSpeed *MoveCount;
     FVelocityY := FVelocityY + FAccelY * UpdateSpeed *MoveCount;
