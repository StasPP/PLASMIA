//------------------------------------------------------------------------------
// Some routines added to discard other units            Modified: 09-Jan-2007
// Modified by Lord Zero
// Contact: xmaas@msn.com
//------------------------------------------------------------------------------
(*----------------------------------------------------------------------------*
...
uses
  Windows, Classes, MMSystem, DirectSound, AsphyreDb;
...
//    m_pWaveFile: CWaveFile;
  ...
  SoundDevice : CSoundManager;
  Sound: array of CSound;
  Filename: Widestring;

  function Initialize(WindowHandle: THandle): Boolean;
  function GetSoundTypeFromFileName(FileName: String): string;
  function AddSoundFromFile(const FileName: WideString): Boolean;
  function Add3DSoundFromFile(const FileName: WideString): Boolean;
  function AddFromASDb(const Key: string; ASDb: TASDb): Boolean;
  function LoadFromASDb(const ASDb: TASDb): Boolean;
...
//------------------------------------------------------------------------------
//Routines added to discard others units by Lord Zero      Modified 09-Jan-2007
...
constructor TAsphyreSound.Create;
begin

SoundDevice := CSoundManager.Create;

end;

destructor TAsphyreSound.Destroy;
begin
...
function TAsphyreSound.Initialize(WindowHandle : THandle): Boolean;
begin

 if Failed(SoundDevice.Initialize(WindowHandle, DSSCL_NORMAL)) then
 begin
...
function TAsphyreSound.GetSoundTypeFromFileName(FileName: String): string;
var sExt: String;
begin
...
function TAsphyreSound.AddSoundFromFile(const FileName: WideString): Boolean;
begin

  SetLength(Sound, Count+1);

  Inc(Count);


  if Failed(SoundDevice.Create(Sound[Count-1], PWideChar(FileName), 0, DS3DALG_DEFAULT)) then
  begin
...
function TAsphyreSound.Add3DSoundFromFile(const FileName: WideString): Boolean;
begin
  ...
  if Failed(SoundDevice.Create(Sound[Count-1], PWideChar(FileName), DSBCAPS_CTRL3D, DS3DALG_DEFAULT)) then
  begin
...
function TAsphyreSound.AddFromASDb(const Key: string; ASDb: TASDb): Boolean;
var
Ext:String;
...
begin
  ...
  Stream := TMemoryStream.Create;
    ...
    Ext := GetSoundTypeFromFilename(Key);

    if Ext = '.wav' then
    begin

      wave := cWaveFile.Create;

      Stream.Seek(0, soFromBeginning);

      wave.Read(stream.Memory, stream.Size, nil);

      SoundDevice.CreateFromMemory(Sound[Count+1], wave.m_pbData, wave.m_dwSize, wave.GetFormat, 0, DS3DALG_DEFAULT, 0 );
...
function TAsphyreSound.LoadFromASDb(const ASDb: TASDb): Boolean;
var
i: Integer;
Key: String;
begin
  ...
  begin
    ...
    Key := ASDb.RecordKey[i];

    if GetSoundTypefromFileName(Key) <> 'not' then
    begin
...
function TAsphyreSound.UnLoad: Boolean;
var
i: Integer;
begin
