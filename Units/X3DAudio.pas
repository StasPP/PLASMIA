...
{*                                                                            *}
{*----------------------------------------------------------------------------*}
{*  $Id: X3DAudio.pas,v 1.3 2006/02/11 19:57:28 clootie Exp $ }
{******************************************************************************}
{                                                                              }
...
const
  ...
  X3DAUDIO_HANDLE_BYTESIZE = 16; // size of instance handle in bytes
...
type
  ...
  X3DAUDIO_EMITTER = record
    ...
    pVolumeCurve: PX3DAudioDistanceCurve;    // volume level distance curve, used only for matrix calculations, NULL specifies the default curve: [0.0f,1.0f], [1.0f,0.0f]
    pLFECurve: PX3DAudioDistanceCurve;       // LFE level distance curve, used only for matrix calculations, NULL specifies the default curve: [0.0f,1.0f], [1.0f,0.0f]
    pLPFDirectCurve: PX3DAudioDistanceCurve; // LPF direct-path coefficient distance curve, used only for LPF direct-path calculations, NULL specifies the default curve: [0.0f,0.0f], [1.0f,0.25f]
    pLPFReverbCurve: PX3DAudioDistanceCurve; // LPF reverb-path coefficient distance curve, used only for LPF reverb-path calculations, NULL specifies the default curve: [0.0f,0.25f], [1.0f,0.25f]
...
procedure X3DAudioCalculate(const Instance: TX3DAudioHandle;
  const pListener: PX3DAudioListener; const pEmitter: PX3DAudioEmitter;
  Flags: LongWord; pDSPSettings: PX3DAudioDspSettings); stdcall; external X3DAudioDLL;
