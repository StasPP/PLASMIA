...
begin
 ...
 case DoCenter of
    True:
      begin
        TexMap(Image, pBounds4sc2(X, Y,
         Image.VisibleSize.X, Image.VisibleSize.Y, ScaleX, ScaleY),
          Color, TexCoord, DrawFx);
      end;
    False:
      begin
        if image<>nil then
           TexMap(Image, pBounds4s2(X, Y,
           Image.VisibleSize.X, Image.VisibleSize.Y, ScaleX, ScaleY),
             Color, TexCoord, DrawFx);
 ...
 { case DoCenter of
    True:
      begin
        TexMap(Image, pBounds4sc2(round(X), round(Y),
         round(Image.VisibleSize.X), round(Image.VisibleSize.Y), ScaleX, ScaleY),
          Color, TexCoord, DrawFx);
      end;
    False:
      begin
        if image<>nil then
           TexMap(Image, pBounds4s2(round(X), round(Y),
           round(Image.VisibleSize.X), round(Image.VisibleSize.Y), ScaleX, ScaleY),
             Color, TexCoord, DrawFx);
