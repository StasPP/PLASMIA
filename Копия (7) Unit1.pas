...
var
     ...
     PostFilter3FlashLights:array[1..10] of TFlashLight;
    ...
    /// Shield
     ...
     LevDials:TStringList;
     DialPage:integer;
     DialMode,ScrollChoosed:Boolean;
     DialHudT:real;
     DialScroll:TMyScroll;

     mb1,mb2,mb3,ScenarioTextBegin,ScenarioTextEnd,Hispeed:shortint;
     ...
     DopImages,Bout,LevelCodes,Levels,Hints,MapsList:TStringList;
     Dialog: TStringList;
     Edit1:String[12];
     SymbN: Single;
     dialnew:boolean;
     StringN, Dial, LogoY, MenuDopY, DialDopY, MapsPage, MapN, DialPicN: Integer;
  ...
  Begin
    for I := 1 to 10 do
      Begin
        if PostFilter3flashLights[I].r<=0 then
        Begin
         PostFilter3flashLights[I].x:=BoomX;
         PostFilter3flashLights[I].y:=BoomY;
         PostFilter3flashLights[I].r:=128;
         break;
        End;
      End;
...
begin
   ...
   //  End;
     ...
     Begin
      ...
      AsphyreTextures1.AddRenderTargets(1, 1024, 1024, false, aqLow, alNone, false);

      PostFilter3TexCoords:=TCNull;
      PostFilter3TexCoords.x:= 0;
      PostFilter3TexCoords.y:= 0;
      PostFilter3TexCoords.w:= 1024;// Device.Width;
      PostFilter3TexCoords.h:= 1024; //Device.Height;
  ...
  Begin
    ...
    Begin
      ...
      if numb<>-1 then
         ...
         1,5..7,16,17,18,19,20,22,26,27,28,30,31..36,39,40,50..54,55..72: Begin
           MyTile:=TTile.Create(Engine);
                    ...
                    // if Objs[numb].Tip=1 then Begin
                      ...
                      if (HiDet=false)and(tip<>70) then
                        ...
                        Begin
                         ...
                         ScaleX:=1.5;
                         ScaleY:=1.5;
                         if (Pos('xdec',Objs[numb].Img)=1)or
                              ...
                              if Objs[numb].parns[k]<>'' then
                                ...
                                Begin
                                 ...
                                 delete(par,1,length(Objs[numb].parns[k])+2);
                                  ...
                                  if (Objs[MyobjN].parns[k]='Color')or(Objs[MyobjN].parns[k]='Message') then
                                  Begin
                                     col:=strtoint(par);
                                     if col>0 then
 ...
 { for I := 0 to 3 do
  ...
  if mdown[mb1] then
   ...
   if (hudt<=0)and(health>0) then
    if (CursorOnCapsule)and(takencapsule<>nil) then
    Begin
       if (mdown[mb1]){or(Mouse.Pressed[mb3])} then
       Begin
         TakeCapsule;
         Droping:=false;
       End;
...
begin
////
  MyCanvas.FillRect(0,0,1024,1024,cRGB1(40,40,100),fxNone);

  ind:=1024/Device.Width;
  indy:=1024/Device.Height/ind;

  ba:=trunc(GameScaleX*400*ResolutionScaleX*ind);

  MyCanvas.DrawStretch(Images.Image['fonar'],0,trunc(512-ba)
            ,trunc(512-ba*indy),trunc(512+ba)
            ,trunc(512+ba*indy),false,false,clWhite4,fxAdd);

 ba:=trunc(GameScaleX*200*ResolutionScaleX*Ind);

 MyCanvas.DrawStretch(Images.Image['fonar'],0,trunc(mx*1024/Device.Width-ba)
            ,trunc(my*1024/Device.Height-ba*indy),trunc(mx*1024/Device.Width+ba)
            ,trunc(my*1024/Device.Height+ba*indy),false,false,clWhite4,fxAdd);
      ...
      Begin
        ...
        Begin
         ...
         Begin
           ...
           MyCanvas.DrawStretch(Images.Image['fonar'],0,trunc(bx*1024/Device.Width-br)
            ,trunc(by*1024/Device.Height-br*indy),
            trunc(bx*1024/Device.Width+br)
            ,trunc(by*1024/Device.Height+br*indy),false,false,crgb4(255,255,255,ba),fxAdd)
   ...
   Begin
      ...
      Begin
       ...
       if tipp<>5 then
        ...
        if tipp=1 then
        Health:=Health-round(3*jj*Power*diff[diffi])
...
var mycolor:Cardinal;
 ...
 __x,__y,__w,__h,_dop,_dop2,_dop3:real;
 _scale,_sc:real;
 imgname,st:string;
begin
  MapBorder;
  ...
  With MyCanvas do Begin
    ...
    Begin
      ...
      DrawStretch(HudImages.Image['mishud'],0,
            trunc((5-256+levelmissionshow)*ResolutionScaleX),
            trunc((250)*ResolutionScaleY2),
            trunc((5+levelmissionshow-85-48)*ResolutionScaleX),
            trunc((250+{64}43)*ResolutionScaleY2),false,false,
            cRGB4(255,255,255,trunc(levelmissionshow)),fxBlend);

      DrawStretch(HudImages.Image['mission'+inttostr(levelmissiontip)],0,
            trunc((6-256+levelmissionshow)*ResolutionScaleX),
            trunc((250)*ResolutionScaleY2),
            trunc((6-256+43{-192}+levelmissionshow)*ResolutionScaleX),
            trunc((250+{64}43)*ResolutionScaleY2),false,false,
            cRGB4(255,255,255,trunc(levelmissionshow)),fxBlend);

      st:=inttostr(levelmission);

      __w:=Fonts[1].TextWidth(st);
      Fonts[1].TextOut(st,
      ({168}116-256+levelmissionshow-24)*ResolutionScaleX-__w/2, {328}270 *ResolutionScaleY2-__h/2,
...
//              round(HudImages.Image[imgname].VisibleSize.y*(1-0.01*(100-health))),
      ...
      if (DialMode=false)and(DialHudT<=0)and(LevDials.Count>0) then Begin
        if (hudt>0) then Begin
         if Hud_currentzone=15 then
           imgname:='Hud10_2'
            else
...
var i,j,k,x1,y1,_x,_y,bx,by,bx2,by2,al,i1,i2,j1,j2,fx1,fx2,fy1,fy2:integer;
...
//               mapspage:=20;
     ...
     {список карт}
      ...
      End;
       ...
       crgb1(10,100,200,trunc(_alf/3)),fxBlend);
   ...
   if not(paused) then
     ...
     Begin
      ...
      _th:=(Globalticks-10*i)/10;
       ...
       _str:=copy(language[IntroStrBegin+(CurrentScreenN)*4+i],1,trunc(length(language[80+i+(CurrentScreenN)*4])*_th));
...
End;
 ...
 Begin
   cursoroncapsule:=true;
   if (RV<400)and(isDone=false) then TakenCapsule:=self;
 End;
...
Begin
 ...
 if (TTile(Sprite).objname='tconv1')and(TTile(Sprite).pars[3]<>1){and(tip>3)}and(keeping=false) then
 Begin
