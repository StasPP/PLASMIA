...
//     CamScale:real;
     ...
     CurrentTrack, TrackVol, conpos:integer;
     Page:byte;
     HudXShift:integer;
    ...
    // Survival:Boolean;
     ...
     LevA :array [1..15,1..2] of Integer;
    /// ССЫЛКА НА ИГРОКА
     _Player:TSprite;
         ...
         if (MapLookObjs[j].ObjX*100 >= MapZones[i].ZoneRect.Left) and
            ...
            begin
              ...
              case MapLookObjs[j].ObjTip of
                1,5,7,10,21,25,26,35: MapLookObjs[j].ObjTip:=-1;
                2,3,8,14..17,33: MapLookObjs[j].ObjTip:=-2;
                4,11,19,9: MapLookObjs[j].ObjTip:=-3;
                -4..0, 27..30: begin end;
                else
                  MapLookObjs[j].ObjTip:=-4;
              end
            end;
       for j := MapZones[i].ZoneRect.Left div 100 to MapZones[i].ZoneRect.Right div 100 do
        for k := MapZones[i].ZoneRect.Top div 100 to MapZones[i].ZoneRect.Bottom div 100  do
         if (j>0)and(j<mapsizeX)and(k>0)and(k<mapsizey) then
          if larr[j,k]<>0 then
            begin
              larr[j,k]:=8;
            end;
...
begin
  ...
  S.LoadFromFile('Data\Locs\Pos.txt');
  ...
  begin
    Str := S[I-1];
    j := Pos(' ',Str);
    LevA[I,2] := StrToInt(copy(str,j+1, length(str)-j));
    LevA[I,1] := StrToInt(copy(str,1, j-1));
  end;
                  ...
                  4:{Windowed:} Begin
                    ...
                    begin
                      ...
                      Device.Windowed:=true
                        ...
                        Device.Windowed:=true;
                        if (Device.Height <> Screen.Height) or
                           (Device.Width  <> Screen.Width) then
                        Device.Windowed:=false;
                        BorderStyle:=bsnone;
                       End;
                  End;
...
///
  ...
  if not((curcbutton = 201)and(menuN=11)) then
    curcbutton:=-1;
  ...
  if (curcbutton = 201)and(menuN=11) then
    j:=201;
 ...
 { if mup[0] then
  ...
  {     if (menun=6)or(menun=19) then
     ...
     Begin
        ...
        if menun=11 then
          if (curbutton=201) then
          begin
             menuready:=false;
             nextmenu:=2; 
             curbutton:=-1;
             //mapN:=1; //mapsPage+curbutton-100;
             ////  Данные карты
             //ReadMapHeader;
          end;
...
begin
    ...
    //    50 *ResolutionScaleX,(300-250+cony )*ResolutionScaleY, cRGB1(255, 255, 255));
      s:=' MN: ' + IntToStr(Menun)+ ' NN: '+IntToStr(NextMenu) + ' T: '+format('%.3f',[GlobalTicks]);
...
begin
  ...
  if ConsoleList.Count>0 then
  begin
    if ConsoleList[ConsoleList.Count-1]<> concom then
        ConsoleList.Add(concom);
  end
    else
      ConsoleList.Add(concom);
...
if not(paused) then
 ...
 if (Paused) or (Active = False) then
  ...
  if Console then
    ...
    Begin
      if key=vk_down then
       ...
       if conpos<0 then
         conpos:=0;
       if conpos>ConsoleList.Count-1 then
         conpos:=ConsoleList.Count-1;
       if ConsoleList.Count>0 then
         concom:=ConsoleList[conpos]
     End;
end;

procedure TMainForm.FormKeyPress(Sender: TObject; var Key: Char);
begin
 if (key='m')and Developer and not console then
   begin
    MapLookMenu:=true;
    MapLookX:=_Player.X;
    MapLookY:=_Player.Y;
    ...
    Begin
       ...
       if dialtime>255 then
        Begin
          ...
          DrawStretch(HudImages.Image['dial2'],0,  trunc((800-485)*ResolutionScaleX),
            trunc((50)*ResolutionScaleY),trunc((800-285)*ResolutionScaleX),
            trunc((50+64)*ResolutionScaleY),false,false,
            cRGB4(255,255,255,trunc(255*(100-Hudt)/100)),fxBlend);

          DrawStretch(HudImages.Image['dial'],0,  trunc((1600-382)*ResolutionScaleX/2),
            trunc((50)*ResolutionScaleY),trunc((1600+1022)*ResolutionScaleX/2),
            trunc((50+128)*ResolutionScaleY),false,false,
            cRGB4(255,255,255,trunc(255*(100-Hudt)/100)),fxBlend)
        End
          else
          Begin
             ...
             DrawStretch(HudImages.Image['dial2'],0,  trunc((800-485)*ResolutionScaleX),
              trunc((50+dialtime/2-127)*ResolutionScaleY),trunc((800-285)*ResolutionScaleX),
              trunc((50+64+dialtime/2-127)*ResolutionScaleY),false,false,
              cRGB4(255,255,255,trunc(dialtime*(100-Hudt)/100)),fxBlend);

             DrawStretch(HudImages.Image['dial'],0,  trunc((1600-382)*ResolutionScaleX/2),
                trunc((50+dialtime/2-127)*ResolutionScaleY),trunc((1600+1022)*ResolutionScaleX/2),
                trunc((50+128+dialtime/2-127)*ResolutionScaleY),false,false,
                cRGB4(255,255,255,trunc(dialtime*(100-Hudt)/100)),fxBlend);
          End;
      ...
      if dialtime<=255 then
        Fonts[1].TextOut(Pnames[dialpicN],
                (339*ResolutionScaleX),(69+dialtime/2-127)*ResolutionScaleY, cRGB1(250, 250, 255, trunc(dialtime*(100-Hudt)/100)))
        else
          Fonts[1].TextOut(Pnames[dialpicN],
                (339*ResolutionScaleX),(69)*ResolutionScaleY, cRGB1(250, 250, 255, trunc(255*(100-Hudt)/100)));

       for i := 1 to StringN do
         if dialtime<=255 then
          Fonts[1].TextOut(Stringarr[i],
            (724*ResolutionScaleX),(53+20*i+dialtime/2-127)*ResolutionScaleY, cRGB1(250, 250, 255, trunc(dialtime*(100-Hudt)/100)))
            else
              Fonts[1].TextOut(Stringarr[i],
                (724*ResolutionScaleX),(53+20*i)*ResolutionScaleY, cRGB1(250, 250, 255, trunc(255*(100-Hudt)/100)));
                         ...
                         if larr[j,k]<8 then
                           DrawStretch(MMImages.Image['mm12'],0,_x-x1 ,_y-y1,
                            sx,sy,1,1,false,false,false,
                             ...
                             DrawStretch(MMImages.Image['mm12'],0,_x-x1 ,_y-y1,
                            sx,sy,1,1,false,false,false,
                             cRGB4(trunc(RedW[5]*0.6),trunc(GreenW[5]*0.6),trunc(Bluew[5]*0.6),250),fxBlend);
                      end;
              ...
              and(abs(MapLookObjs[i].ObjY-MapLooky/100)<25) then
               Begin
                ...
                {  if MapLookObjs[i].ObjEnabled then
                  ...
                  case MapLookObjs[i].ObjTip of
                        -1,1,7,10,35: DrawStretch(MMImages.Image['mmfon'+litera],0,_x-x1-sx ,_y-y1-sy,
                        ...
                        4,9,11,19,-3: DrawStretch(MMImages.Image['mmfon4'+litera],0,_x-x1-sx ,_y-y1-sy,
                            sx*3,sx*3,1,1,false,false,false,col,fxBlend);
                      //  9: DrawStretch(MMImages.Image['mmfon4'+litera],0,_x-x1-sx*0.7 ,_y-y1-sy*0.7,
                      //      sx*3*0.7,sx*3*0.7,1,1,false,false,false,col,fxBlend);
                       // 9: DrawStretch(MMImages.Image['obj9'],0,_x-x1-sx*1.5 ,_y-y1-sy*1.5,
                       //     sx*3*1.5,sx*3*1.5,1,1,false,false,false,col,fxBlend);
...
var _h,_h2,_w, _x, _y, _alf2,_th, cx, cy, asz, cpx, cpy, _cpx, _cpy:single;
    _alf,i,j,k,_red, aaz:integer;
    l,kn:byte;
    _str:string;
begin
if inmenu then
Begin
 ...
 Begin
   if _alf >= 200 then
   begin
     if ScenTicks < 100 then
       ScenTicks := ScenTicks + LagCount*0.7
       Else
          ScenTicks := 100;
   end;
     i := (ScenarioTextEnd-ScenarioTextBegin);
     j := trunc(ScenTicks*i/100)+1;
     k := length(scenario[j+ScenarioTextBegin]);

     menuticks:=menuticks+lagcount*0.001;
       if menuticks>=2*pi then
         menuticks:= menuticks - 2*pi;
   ...
   FillRect(0,trunc(525*resolutionscaleY2),
       trunc(525*ResolutionScaleX),
       trunc((380)*resolutionscaleY2),
       crgb1(10,100,200,trunc(_alf/2.5)),fxBlend);

   FillRect(trunc(600*ResolutionScaleX),trunc(450*resolutionscaleY2),
       trunc(1600*ResolutionScaleX),
       trunc((200+50*({ScenTicks*(i-4)/100)}
       ScenarioTextEnd-ScenarioTextBegin-4))*resolutionscaleY2),
       crgb1(10,100,200,trunc(_alf/2.5)),fxBlend);

   cx := 250*resolutionscaleX;
   cy := 710*resolutionscaleY2;
   asz := 400*resolutionscaleX;
   aaz := round(asz / 30);
   _x:=680*ResolutionScaleX;

   Fonts[1].Scale:=(ResolutionScaleY2)*0.75/normwscale; /// new060115

   if i > 0 then
   begin
      k := trunc((ScenTicks - (j-1)*100/i)*i);
      k := trunc(k/100*length(scenario[j+ScenarioTextBegin]));
   end;

   for I := ScenarioTextBegin+2 to ScenarioTextEnd do
   Begin
      //GlobalTicks:=GlobalTicks+Lagcount;
      if I-ScenarioTextBegin > j then
         break;
      if I-ScenarioTextBegin  < j then
        _str := scenario[i]
         else
           _str:=copy(scenario[i], 1, k);

      //_w:=trunc(Fonts[1].TextWidth(_str)/ 2);

      Fonts[1].TextOut(_str,_x,
   ...
   End;
    ...
    //// НОМЕР ГЛАВЫ
       ...
       _w:=trunc(Fonts[1].TextWidth(_str)/ 2);
       Fonts[1].TextOut(_str, _x+300*ResolutionScaleX-_w,
          (90*resolutionscaleY2),
          cRGB1(255, 255, 255, trunc(_alf2*255)));

      //// НАЗВАНИЕ
//       Fonts[1].Scale:=ResolutionScaleY2*1ю2;
       _str:=scenario[ScenarioTextBegin+1];             {QQQQQ}
     ...
     // _w:=35*resolutionscaleY-35*resolutionscaleY2;
      ...
      FillRect(trunc(40*resolutionscaleX),
      trunc(30*resolutionscaleY2),
      trunc((370)*ResolutionScaleY2),
      trunc((370)*ResolutionScaleY2),
       crgb1(10,100,200,trunc(5+5*sin(menuticks*25))),fxBlend);
      ...
      DrawRotateStretchC(Images.Image['aster'],0,
      trunc(cX+asz*0.07),
      trunc(cY+asz*0.07),
      trunc(asz*1.1),
      trunc(asz*1.1), menuticks,
      cRGB4(0,0,0,trunc(0.7*_alf)),
      fxBlend);
      ...
      trunc(asz),
      trunc(asz), menuticks,
     // cRGB4(105,205,225,_alf),      
      cRGB4(85,195,215,_alf),
      fxBlend);
   ...
   begin
      cpx :=  round(cx + 0.005*aSz*LevA[i,1] * sin( -menuticks + LevA[i,2]*pi/180 + pi/2));
      cpy :=  round(cy + 0.005*aSz*LevA[i,1] * cos( -menuticks + LevA[i,2]*pi/180 + pi/2));


      if i > 1 then

        for j := - trunc(2*resolutionscaleX) to trunc(2*resolutionscaleX) do
        begin
           WULine(Point2(cpx+j,cpy),Point2(_cpx+j,_cpy),
               cRGB1(155,255,155,_alf), cRGB1(155,255,155,_alf), fxBlend);

           WULine(Point2(cpx,cpy+j),Point2(_cpx,_cpy+j),
               cRGB1(155,255,155,_alf), cRGB1(155,255,155,_alf), fxBlend);
        end;
      ...
      DrawStretch(MMImages.Image[_str],0,
      round(cpx) - aaz,
      round(cpy) - aaz,
      round(cpx) + aaz,
      round(cpy) + aaz,false,false,
      cRGB4(255,255,255,trunc(_alf*(0.75+0.25*sin(I+menuticks*100)))),
      fxAdd);
      ...
      begin
        Fonts[1].Scale:=ResolutionScaleY2*0.8;
        _str:=scenario[ScenarioTextEnd+1];  
        _w:=trunc(Fonts[1].TextWidth(_str));
         
        FillRect(round(cpx)+aaz,round(cpy),
       round(_w) + aaz*2,
       round(26*ResolutionScaleY2),
       crgb1(10,70,150,trunc(_alf/1.5)),fxBlend);
         
        Fonts[1].TextOut(_str, round(cpx)+aaz*2,
          round(cpy),
          cRGB1(255, 255, 255, trunc(_alf2*255)));
      end;
 ...
 {  FillRect(0,trunc(75*resolutionscaleY2),
   ...
   FillRect(0,trunc(500*resolutionscaleY2),
       trunc(475*ResolutionScaleX),
       trunc((400)*resolutionscaleY2),
       crgb1(10,100,200,trunc(_alf/2.5)),fxBlend);

   FillRect(trunc(525*ResolutionScaleX),trunc(300*resolutionscaleY2),
       trunc(1600*ResolutionScaleX),
       trunc((200+50*({ScenTicks*(i-4)/100)}
       ScenarioTextEnd-ScenarioTextBegin-4))*resolutionscaleY2),
       crgb1(10,100,200,trunc(_alf/2.5)),fxBlend);
   ...
   FillRect(trunc(725*ResolutionScaleX),trunc(850*resolutionscaleY2),
       trunc(1600*ResolutionScaleX),
       trunc(170*resolutionscaleY2),
       crgb1(10,100,200,trunc(_alf/2.5)),fxBlend);
           
   cx := 225*resolutionscaleX;
   cy := 685*resolutionscaleY2;
   asz := 400*resolutionscaleX;
   aaz := round(asz / 30);
   _x:=560*ResolutionScaleX;
   ...
   Begin
      ...
      Fonts[1].TextOut(_str,_x,
          (385+50*(i-ScenarioTextBegin-2))*resolutionscaleY2,
          cRGB1(255, 255, 255, trunc(_alf2*255)));
  ...
  { for I := ScenarioTextBegin+1 to ScenarioTextEnd do
   ...
   End;   }
    ...
    _h:=0;//(Menus[MenuN].h*menut*0.01)*ResolutionScaleY2*2;
      ...
      DrawRotateStretchC(MenuImages.Image['plasmia'],0,
      trunc(cX+asz*0.07),
      trunc(cY+asz*0.07),
      trunc(asz*1.1),
      trunc(asz*1.1), 0,
     ...
     // cRGB4(105,205,225,_alf),      
      ...
      for k := 0 to 1 do    
      DrawStretch(MMImages.Image['mm10'],0,
      round(cx + asz*0.325) - aaz,
      round(cy - asz*0.325) - aaz,
      round(cx + asz*0.325) + aaz,
      round(cy - asz*0.325) + aaz,false,false,
      cRGB4(255,255,255,trunc(_alf*(0.85+0.15*sin(menuticks*200)))),
      fxAdd);

   
                                         
       Fonts[1].Scale:=ResolutionScaleY2*1.4;
       _str:=scenario[ScenarioTextBegin];
       if Cheater then
         _str:=_str+language[180];
       _w:=trunc(Fonts[1].TextWidth(_str)/ 2);
       Fonts[1].TextOut(_str, _x-_w + 400*ResolutionScaleX,
          (90*resolutionscaleY2),
          cRGB1(255, 255, 255, trunc(_alf2*255)));

       Fonts[1].Scale:=ResolutionScaleY2*0.9;
       _str:= scenario[ScenarioTextEnd+1]; ///language[178]+IntToStr(Allscore)+language[179];
       _w:=trunc(Fonts[1].TextWidth(_str));
       ...
       DrawStretch(MenuImages.Image['lev_final'],0,
      trunc(40*resolutionscaleX),
      trunc(30*resolutionscaleY2),
      trunc((410)*ResolutionScaleY2),
      trunc((400)*ResolutionScaleY2),false,false,
      ...
      for I := 1 to 5 do 
      if scenticks >= I*10 then
      Begin
        DrawStretch(MenuImages.Image['big_medal'+inttostr(i)], 0,
          trunc((750+165*(i-1))*resolutionscaleX),
          trunc(890*resolutionscaleY2),
          trunc(120*ResolutionScaleY2),
          trunc(120*ResolutionScaleY2),1,1,false,false,false,
          cRGB4(255,255,255,_alf),
          fxBlend);
        ...
        Fonts[1].TextOut(_str,(165*(i-1)+850)*resolutionscaleX,
            (970)*resolutionscaleY2,
            cRGB1(255, 255, 255, trunc(_alf2*255)));

        Fonts[1].Scale:=ResolutionScaleY2*0.8;
        _str:=Language[78+i];
        _w:=trunc(Fonts[1].TextWidth(_str)/ 2);
        Fonts[1].TextOut(_str,(805+165*(i-1))*resolutionscaleX -_w, 
          862*resolutionscaleY2,
          cRGB1(255, 255, 255, trunc(_alf)));
      End;  
      ...
      End;    
        ...
        if curcbutton = 201 then
            curcbutton := -1;                                      
                //  extras[2]  := true;    extras[3]  := true;  extras[1]  := true;
        if extras[1] then
        Begin
          j := 30;
          if extras[2] or extras[3] then
            j:= 0;
        
          Fonts[1].Scale:=ResolutionScaleY2*0.75;

          _str:=language[224]+ExtrM[1];
          _w:=trunc(Fonts[1].TextWidth(_str));   
          if Fonts[1].TextWidth( language[225]) > _w then
             _w := trunc(Fonts[1].TextWidth( language[225]));

          
          _str:= language[225];

          k :=0;  
          if (mx < _w+  40*ResolutionScaleX) then
           if (my > (950+j)*resolutionscaleY2) then 
           if (my < (1010+j)*resolutionscaleY2) then
           begin
              CurCButton := 201;
              k := 90;
           end;
          ...
          FillRect(0,
            trunc((950+j)*resolutionscaleY2),
            trunc(_w + 40*ResolutionScaleX),
            trunc((60*resolutionscaleY2)),
            crgb1(10,100,200,trunc(_alf/4.5+k)),fxBlend);

          Fonts[1].TextOut(_str, 20*ResolutionscaleX,
            ((980+j)*resolutionscaleY2),
            cRGB1(255, 255, 255, trunc(_alf2*255)));
            
          _str:=language[224]+ExtrM[1];
          Fonts[1].TextOut(_str, 20*ResolutionscaleX,
            ((955+j)*resolutionscaleY2),
            cRGB1(255, 255, 255, trunc(_alf2*255)));
        ...
        Begin
          ...
          _str:=language[224]+ExtrM[2];
          _w:=trunc(Fonts[1].TextWidth(_str));   
          if Fonts[1].TextWidth( language[226]) > _w then
             _w := trunc(Fonts[1].TextWidth( language[226]));
             
          _str:= language[226];

          k :=0;  
          if (mx < _w+  40*ResolutionScaleX) then
          if (my > (1030)*resolutionscaleY2) then 
          if (my < (1090)*resolutionscaleY2) then
          begin
              CurCButton := 201;
              k := 90;
          end;
          ...
          FillRect(0,
            trunc(1030*resolutionscaleY2),
            trunc(_w+40*ResolutionScaleX),
            trunc((60*resolutionscaleY2)),
            crgb1(10,100,200,trunc(_alf/4.5+k)),fxBlend);

          Fonts[1].TextOut(_str, 20*ResolutionscaleX,
            (1060*resolutionscaleY2),
            cRGB1(255, 255, 255, trunc(_alf2*255)));
            
          _str:=language[224]+ExtrM[2];  
          Fonts[1].TextOut(_str, 20*ResolutionscaleX,
            (1035*resolutionscaleY2),
            cRGB1(255, 255, 255, trunc(_alf2*255)));
       ...
       if extras[3] then
        Begin
          ...
          _str:=language[224]+ExtrM[3];
          _w:=trunc(Fonts[1].TextWidth(_str));   
          if Fonts[1].TextWidth( language[227]) > _w then
             _w := trunc(Fonts[1].TextWidth( language[227]));
             
          _str:= language[227];
          
          k :=0;  
          if (mx < _w+  40*ResolutionScaleX) then
           if (my > (1030+j)*resolutionscaleY2) then 
           if (my < (1090+j)*resolutionscaleY2) then
           begin
              CurCButton := 201;
              k := 90;
           end;
          ...
          FillRect(0,
            trunc((1030+j)*resolutionscaleY2),
            trunc(_w + 40*ResolutionScaleX),
            trunc((60*resolutionscaleY2)),
            crgb1(10,100,200,trunc(_alf/4.5+k)),fxBlend);

          Fonts[1].TextOut(_str, 20*ResolutionscaleX,
            ((1060+j)*resolutionscaleY2),
            cRGB1(255, 255, 255, trunc(_alf2*255)));
            
          _str:=language[224]+ExtrM[3];  
          Fonts[1].TextOut(_str, 20*ResolutionscaleX,
            ((1035+j)*resolutionscaleY2),
            cRGB1(255, 255, 255, trunc(_alf2*255)));
        End;
   ...
   if not(paused) then
     ...
     Begin
       DrawStretch(Images2.Image[inttostr(CurrentScreenN-1)],0,
          ...
          cRGB4(255,255,255,trunc(255-_alf2)), fxBlend);
