...
type
     ...
     TAIState = (AIHunt, AIGoto, AIStop,AIRunaway);
     TParType = (pFire, plaserpoint,pPlasmid2, pWarer,pShield, psun,
                psun2, pcircle, pTrasser, pExplode, pExplode2, pCol, pCol2,
                pPlasmid, pHelix);
     ...
     private
         ...
         // EnmChilds:TList;
          ...
          TargetX,TargetY,TargetDest:integer;
...
const
  ...
  IntroX:array[1..8] of Integer =(100,50,150, 50,  80,  250, 50,400);
  IntroY:array[1..8] of Integer =(100,50,100,500,1000,1000,1000,950);
...
var
    ...
    /// Shield
     ...
     mb1,mb2,ScenarioTextBegin,ScenarioTextEnd,Hispeed:shortint;

     Loadthread:TLoadThread;
     
     /// ТЕКСТ!
     lang,smessage,dialpicture:string;
     smessagetime,dialtime:single;
     Language,Scenario,ItemsList,BonusList,
     DopImages,Bout,LevelCodes,Levels,Hints:TStringList;
     Dialog: TStringList;
     Edit1:String[12];
     SymbN: Single;
     dialnew:boolean;
     StringN, Dial, LogoY: Integer;
    ...
    Mmap:array[1..200,0..4]of integer;//MiniBitMap:TBitmap;
   ...
   //  messt:real;
     lastbutton:integer;
     Waittofade,resdop,Globalticks,maplookdopy:real;
     MapLookX,MapLookY,MapLookT:real;
    ...
    /// УРОВЕНЬ, Очки
     ...
     MapLookObjs:array[1..256] of TMapLookObjs;
    ...
    /// POST-FILTER
     ...
     virtualH,virtualW,deltaY:integer;
     ...
     Menuticks:single;
     MenuN,NextMenu,CurButton,CurCButton,NewKey,CurrentScreenN:integer;
     MenuT:real;
     Menus:array[0..20] of TMyMenu;
...
procedure SparkEff2(Posx,PosY:real; PType:TParType);
var i:integer;
Begin
     for i := 1 to 25 do
      with  TParticle.Create(Mainform.Engine) do
...
const LangStringsCount=128;
      ScenarioStringsCount=128;
...
///
  ...
  if fileexists('Saves\Slot'+inttostr(slot)+'\CheckPoint.loc') then
  Begin

  mapfile.LoadfromFile('Saves\Slot'+inttostr(slot)+'\CheckPoint.loc');
...
CurrentScreenN:=1;
 if Hidet=true then
    AGraphics.FileName:='Data\Graphics\Intro_HD.asdb'
     else
      AGraphics.FileName:='Data\Graphics\Intro_Ld.asdb';
  ...
  Begin
    ...
    Begin
      ...
      if numb<>-1 then
         ...
         1,5..7,16,17,18,19,20,22,26,27,28,30,31..36,39,40,50..54,55..70: Begin
           MyTile:=TTile.Create(Engine);
          ...
          End;
             ...
             End;
              ...
              Begin
                 ...
                 Begin
                   ...
                   // MyObjN:=numb;
                    ...
                    if (EnmName='enm1')or(EnmName='enm2') then
                    Begin
                      ScaleX:=ScaleX*1.2;
                      Bigger:=true;
                    End;
                   ...
                   if (EnmName='enm3')or(EnmName='enm6')or(EnmName='enm8')
                    or(EnmName='enm9')or(EnmName='enm10') then
                    Begin
                      if (ppp[1]<>0) then
                      Begin
                        enmweap:=ppp[1];
              ...
              10,11,13: Begin
                 if not((difficulty>1)and(Objs[numb].Tip=11)) then
...
procedure TMainForm.LoadSettings;
  ...
  'Language: ','MaxFPS: ','LowAnim: ');// НАСТРОЙКИ!!!
    ...
    Begin
     ...
     if hud2[Hud_currentzone].hudtype=8 then
       ...
       End;
        ...
        if hud2[Hud_currentzone].hudtype=5 then  CloseInv2;
    End;
///
end;
 ...
 for i := 0 to 40 do
    ...
    begin
      ...
      _y:=(Hud3[hud_hotzones[i].no].cy)+hud_hotzones[i].y*_scale*ResolutionScaleY;
       ...
       Begin
         if ((hud_currentzone<=33) and (MagzLev>=(hud_currentzone-24)/ 3))or
            ((hud_currentzone>33) and (MagzLev>=(hud_currentzone-33)/ 2))  then
            Begin
  ...
  if mdown[0] then Begin
    ...
    CloseInv3;
      ...
      if (Hud_Currentzone>24)and(Hud_currentZone<=33) then
       Begin
        if (MagzLev>=(hud_currentzone-24)/ 3)  then
        Begin
       ...
       Begin
        if (MagzLev>=(hud_currentzone-33)/ 2)  then
        Begin
          if MagzObjs[hud_currentzone-24].cost>globalscore then
            AddHint(language[42],resolutionscaleY,mx,my,1)
            else
  ...
  Begin
    if ((my>310*resolutionscaleY2)and
     (my<340*resolutionscaleY2)) then
      Begin
        if (mx>500*resolutionscaleX)and
           (mx<550*resolutionscaleX) then
            if page>0 then j:=1;

        if (mx>1050*resolutionscaleX) and
           (mx<1100*resolutionscaleX) then
            if page<2 then j:=2;
      End;
  End;
       ...
       Begin
        if ((my>(590+Menus[MenuN].buttons[i].y)*resolutionscaleY2)and
            ...
            Begin
             case i of
              0:Begin
                  SoundVolume:=trunc(((mx-xmin)/(xmax-xmin))*10+0.5)*10;
                End;
              1:Begin
                  MusVolume:=trunc(((mx-xmin)/(xmax-xmin))*10+0.5)*10;
                End;
              2:mSpd:=trunc(((mx-xmin)/(xmax-xmin))*4+1.5);
             end;
  ...
  Begin
       ...
       Begin
        if ((my>(590+Menus[MenuN].buttons[i].y)*resolutionscaleY2)and
            ...
            Begin
             case i of
              0:Begin
                SoundVolume:=trunc(((mx-xmin{-20})/(xmax-xmin{+20}))*10+0.5)*10;
                {if SoundVolume>100 then
                    SoundVolume:=100;
                 if SoundVolume<0 then
                    SoundVolume:=0; }
              ...
              End;
              1:Begin
                MusVolume:=trunc(((mx-xmin{-20})/(xmax-xmin{+20}))*10+0.5)*10;
               { if MusVolume>100 then
                    MusVolume:=100;
                 if MusVolume<0 then
                    MusVolume:=0;  }
     ...
     Begin
        ...
        if nextmenu=-5 then
         ...
         if (nextmenu=1)and(menun=6) then
          Begin
           SetVolumes;
          // SetMusVolumes;
           SaveSettings;
...
begin
 ...
 { for I := 0 to 3 do
  ...
  { if Mouse.Pressed[2] then
   ...
   if (inventory=true) and(Hudt=100)
   then Begin
     MouseInventory;
   End;
...
begin
     ...
     Fonts[1].TextOut('WORLD X: '+IntToStr(trunc(Engine.WorldX))+' Y: '+IntToStr(trunc(Engine.WorldY))
        +' Scale: '+s,50 *ResolutionScaleX, (50-250+cony )*ResolutionScaleY, cRGB1(250, 250, 55));
...
begin
     ...
     x0:=-layerX*0.1*ResolutionScaleX+BoomX*ResolutionScaleX;
     y0:=-layerY*0.1*ResolutionScaleY+BoomY*ResolutionScaleY;
     ...
     x0:=-layerX*0.25*ResolutionScaleX+BoomX*ResolutionScaleX;
     y0:=-layerY*0.25*ResolutionScaleY+BoomY*ResolutionScaleY;
     while x0>Device.Width do x0:=x0-Device.Width;
     while x0<0 do x0:=x0+Device.Width;
     while y0>Device.Height do y0:=y0-Device.Height;
     while y0<0 do y0:=y0+Device.Height;
     ...
     x0:=-layerX*0.35*ResolutionScaleX+BoomX/1.5*ResolutionScaleX;
     y0:=-layerY*0.35*ResolutionScaleY+BoomY/1.5*ResolutionScaleY;
     while x0>Device.Width do x0:=x0-Device.Width;
     while x0<0 do x0:=x0+Device.Width;
     while y0>Device.Height do y0:=y0-Device.Height;
     while y0<0 do y0:=y0+Device.Height;
     ...
     x0:=-layerX*0.5*ResolutionScaleX+BoomX/2*ResolutionScaleX;
     y0:=-layerY*0.5*ResolutionScaleY+BoomY/2*ResolutionScaleY;
     while x0>Device.Width do x0:=x0-Device.Width;
     while x0<0 do x0:=x0+Device.Width;
     while y0>Device.Height do y0:=y0-Device.Height;
     while y0<0 do y0:=y0+Device.Height;
   ...
   Begin
      ...
      Begin
       ...
       if tipp<>5 then
        ...
        if tipp=1 then
        Health:=Health-round(3*jj*Power*diff[difficulty])
         else if tipp<>5 then Health:=Health-round(jj*Power);
...
begin
    ...
    if (tip=58) then
     ...
     and (_player.Y+128>Y+subrect.top)and(_player.Y+128<Y+subrect.Bottom)then
        Begin
         ...
         Begin
          clr:=clr-movecount*0.1;
          PlusClr:=PlusClr+MoveCount*0.1;
    ...
    if ((tip=7)or(tip=22))and(lakmus) then
    Begin
      if (abs(x+sizexd2-gamcurx)<sizexd2)and (abs(y+sizeyd2-gamcury)<sizeyd2)then
      Begin
    ...
    Begin
      ...
      Begin
        cursoroncapsule:=true;
        if (RV<400)and(keepitm=false) then
          PushTile:=self;
      End;
    End;
  ...
  sz:=sizexdiv2;
   if enmName='Boss1' then
    ...
    Begin
        ...
        Mainform.BoomPhys(trunc(X+SizeXdiv2),trunc(Y+SizeYdiv2),3,SizeYdiv2+100,1);
        MiniExplodeEff(x+SizeXdiv2,y+SizeYdiv2,PExplode);
    ...
    Begin
        ...
        // ExplodeEff(x+SizeXdiv2,y+SizeYdiv2,1,PExplode);
        SparkEff2(x+SizeXdiv2,y+SizeYdiv2, pFire);
        lif:=true;
    End
    else
    if EnmName= 'enm5' then
...
begin
 ...
 Begin
  ...
  { if (Enmname='enm5') then
   ...
   if (abs(_Player.X+128-X+Sizexdiv2)<1600+EnmDopInt)and(abs(_Player.Y+128-Y+Sizeydiv2)<1200+EnmDopInt) then
        AI
         else AIstate:=aihunt;
   inWall:=true;
      ...
      Begin
         ...
         Begin
           ...
           if TCapsule(sprite).tip=4 then
            if TCapsule(sprite).Impulse1.ImpPower>3 then
            Begin
    ...
    gethurt:=true
      ...
      if EnmSubCount>0 then
       ...
       ///  else enmhealth:=enmhealth-1;
        if hieffs then
        Begin
          SparkEff2(TArmosprite(sprite).x,TArmosprite(sprite).y, pFire);
  ...
  Begin
     ...
     // if Enmname='enm1' then ppalf:=-ppalf;
      ...
      if (Enmname='enm3')or(Enmname='enm4')or(Enmname='enm5')
         or(Enmname='enm42')or(Enmname='enm43')then
...
if GunsCount>0 then
 ...
 XX := trunc(Guns[i].X);
 YY := trunc(Guns[i].Y);
 if (EnmRockets=true)and(i<3) then
 Begin
  if pos('Boss',EnmName)>=1 then
      superrock:=true;
 ...
 End else
    ...
    //  ImageName := 'Box1';
      ...
      Begin
        ...
        ArmoPower:=round(EnmSpower*diff[difficulty]);
        DrawFx:=fxAdd;
...
begin
    ...
    if (targetdest>EnmBody.radius)and(targetdest<1600)and(health>0) then
    Begin
...
begin
     // load all images from ASDb
      ...
      ResolutionScaleY2:=Device.Height/1200;

      normWScale:=(ResolutionScaleY2/ResolutionScaleY+1)/2;
...
begin
  GameScaleX:=GameScaleX-0.03*Lagcount;
...
begin
   ...
   if (inventory=true) then
      if (Hud_CurrentZone=1) then
        ...
        Fonts[1].Scale:=ResolutionScaleY*2.5;
...
/// MUSIC
     ...
     trackname:='Data\Music\'+tracknames[level]+'.mp3';//inttostr(1+random(4));  {TEST! TEST! TEST!}

     trackVol:=100;
     if Fileexists(('Data\Music\'+tracknames[level]+'.vol')) then
     Begin
      s:=TStringlist.Create;
      s.LoadFromFile('Data\Music\'+tracknames[level]+'.vol');
...
var mycolor:Cardinal;
 _x,_y,_h,_w,i,j,k,X1,Y1:integer;
 __x,__y,__w,__h,_dop,_dop2:real;
 _scale,_sc:real;
 imgname,st:string;
begin
  MapBorder;
  ...
  With MyCanvas do Begin
    ...
    Rectangle(0,0,round(VirtualW*ResolutionScaleX),round(50*ResolutionScaleY),Mycolor,MyColor,FxBlend);

    Rectangle(0,round((VirtualH-50)*ResolutionScaleY),round(VirtualW*ResolutionScaleX),round(51*ResolutionScaleY)+1,Mycolor,MyColor,FxBlend);

    if smessagetime>0 then
    Begin
       Fonts[1].Scale:=ResolutionScaleY*2;
    ...
    Begin
      ...
      DrawStretch(HudImages.Image['shieldhud'],0,
            trunc((5-256+shieldshow)*ResolutionScaleX),
            trunc((300)*ResolutionScaleY2),
            trunc((5+shieldshow-85)*ResolutionScaleX),
            trunc((300+{64}43)*ResolutionScaleY2),false,false,
            cRGB4(255,255,255,trunc(shieldshow)),fxBlend);

      DrawStretch(HudImages.Image['shieldicon'],0,
            trunc((6-256+shieldshow)*ResolutionScaleX),
            trunc((300)*ResolutionScaleY2),
            trunc((6-256+43{-192}+shieldshow)*ResolutionScaleX),
            trunc((300+{64}43)*ResolutionScaleY2),false,false,
            cRGB4(redw[shieldcolor],greenw[shieldcolor],bluew[shieldcolor],trunc(shieldshow)),fxBlend);

      st:=inttostr(trunc(Shieldtime));
      while length(St)<3 do
       st:='0'+st;

      __w:=Fonts[1].TextWidth(st);
      Fonts[1].TextOut(st,
      ({168}116-256+shieldshow)*ResolutionScaleX-__w/2, {328}320 *ResolutionScaleY2-__h/2,
    ...
    Begin
       ...
       if SymbN<Length(Dialog[StringN+Dial*5]) then
        ...
        if dialtime>255 then
        Begin
          DrawStretch(HudImages.Image[dialpicture],0,  trunc((700-328)*ResolutionScaleX),
            trunc((60)*ResolutionScaleY),trunc((700-128)*ResolutionScaleX),
            trunc((60+200)*ResolutionScaleY),false,false,
            cRGB4(255,255,255,255),fxBlend);

          DrawStretch(HudImages.Image['dial'],0,  trunc((1400-312)*ResolutionScaleX/2),
            trunc((50)*ResolutionScaleY),trunc((1400+1112)*ResolutionScaleX/2),
        ...
        End
          ...
          Begin
             ...
             DrawStretch(HudImages.Image[dialpicture],0,  trunc((700-328)*ResolutionScaleX),
              trunc((60+dialtime/2-127)*ResolutionScaleY),trunc((700-128)*ResolutionScaleX),
              trunc((60+200+dialtime/2-127)*ResolutionScaleY),false,false,
              cRGB4(255,255,255,trunc(dialtime)),fxBlend);

              DrawStretch(HudImages.Image['dial'],0,  trunc((1400-312)*ResolutionScaleX/2),
                trunc((50+dialtime/2-127)*ResolutionScaleY),trunc((1400+1112)*ResolutionScaleX/2),
        ...
        for i := 1 to StringN do
         if dialtime<=255 then
          Fonts[1].TextOut(Stringarr[i],
            (659*ResolutionScaleX),(53+20*i+dialtime/2-127)*ResolutionScaleY, cRGB1(250, 250, 255, trunc(dialtime)))
            else
              Fonts[1].TextOut(Stringarr[i],
                (659*ResolutionScaleX),(53+20*i)*ResolutionScaleY, cRGB1(250, 250, 255, 225));
  ...
  End;
   ...
   Begin
    ...
    Fonts[1].TextOut(IntToStr(Weapons[currentweapon].Count),
      (1600-150-_h)*ResolutionScaleX, 30 *ResolutionScaleY2,
      cRGB1(255, 255, 255,255-round(Hudt*255/100)));

    if Radar then
     Fonts[1].TextOut(IntToStr(round(Health))+'%',
      100*ResolutionScaleX, 10*resdop *ResolutionScaleY2,
      cRGB1(255, 255, 255,255-round(Hudt*255/100)))
    else
    Fonts[1].TextOut(IntToStr(round(Health))+'%',
      150*ResolutionScaleX, 30 *ResolutionScaleY2,
      cRGB1(255, 255, 255,255-round(Hudt*255/100)));
   ...
   End;
    ...
    if cursoroncapsule=true then Begin
      ...
      if (TakeBox<>nil)or(TakenCapsule<>nil)or(takencol<>nil)or(Pushtile<>nil) then
       DrawStretch(HudImages.Image['Cursor_take'],0, trunc(Mx-25*ResolutionScaleX*normwscale),
      trunc(My-25*ResolutionScaleY*normwscale), trunc(Mx+25*ResolutionScaleX*normwscale),
      trunc(My+25*ResolutionScaleY*normwscale),false,false,
      clWhite4,fxBlend)
       else
          DrawStretch(HudImages.Image['Cursor_nottake'],0, trunc(Mx-25*ResolutionScaleX*normwscale),
      trunc(My-25*ResolutionScaleY*normwscale), trunc(Mx+25*ResolutionScaleX*normwscale),
      trunc(My+25*ResolutionScaleY*normwscale),false,false,
      clWhite4,fxBlend)
    End else
     if (inventory2=false)and(stopmenu=false)and(inventory3=false)and(hintmenu=false)and(maplookmenu=false) then
    DrawStretch(HudImages.Image['Cursor'],0, trunc(Mx-15*ResolutionScaleX*normwscale),
      trunc(My-15*ResolutionScaleY*normwscale), trunc(Mx+15*ResolutionScaleX*normwscale),
      trunc(My+15*ResolutionScaleY*normwscale),false,false,
...
begin
 ...
 Begin
      ...
      Begin
        if isRight then
         __x:=((virtualW+xmin+round(Hudt2/100*(xmax-xmin)))*ResolutionscaleX)
            else
...
for I := 1 to 10 do
 Begin
      ...
      Begin
        if isRight then
         __x:=((virtualW+xmin+round(Hudt3/100*(xmax-xmin)))*ResolutionscaleX)
            else
              __x:=((xmin+round(Hudt3/100*(xmax-xmin)))*ResolutionscaleX);
        if isBottom then
...
var i,j,k,x1,y1,_x,_y,bx,by,bx2,by2,al,i1,i2,j1,j2:integer;
    _dop,_dop2,sx,sy:real;
    needdraw:boolean;
    hcount,sdv:byte;
    hstr:array[1..3]of string;
    col,col2:tcolor4;
begin
///
///
              ...
              and(abs(MapLookObjs[i].ObjY-MapLooky/100)<25) then
               Begin
                     ...
                     case MapLookObjs[i].ObjTip of
                        1,5..7,10: needdraw:=mapshow1;
                        2,3,8,14..17: needdraw:=mapshow2;
                        4,11,19,9: needdraw:=mapshow3;
                     end;
                 ...
                 Begin
                  ...
                  case MapLookObjs[i].ObjTip of
                        1,7,10: DrawStretch(MMImages.Image['mmfon'],0,_x-x1-sx ,_y-y1-sy,
                            sx*3,sx*3,1,1,false,false,false,col,fxBlend);
                        5: DrawStretch(MMImages.Image['mmfon2'],0,_x-x1-sx ,_y-y1-sy,
                            sx*3,sx*3,1,1,false,false,false,col,fxBlend);
                        6: DrawStretch(MMImages.Image['mmfon2'],0,_x-x1-sx ,_y-y1-sy,
                            sx*3,sx*3,1,1,false,true,false,col,fxBlend);
                        2,3,8,14..17: DrawStretch(MMImages.Image['mmfon3'],0,_x-x1-sx ,_y-y1-sy,
                            sx*3,sx*3,1,1,false,false,false,col,fxBlend);
                        20: DrawStretch(MMImages.Image['mmfon'],0,_x-x1-sx ,_y-y1-sy,
                            sx*3,sx*3,1,1,false,false,false,clwhite4,fxBlend);
                        4,11,19,9: DrawStretch(MMImages.Image['mmfon4'],0,_x-x1-sx ,_y-y1-sy,
                            sx*3,sx*3,1,1,false,false,false,col,fxBlend);
                  end;


                    DrawStretch(MMImages.Image['Obj'+inttostr(MapLookObjs[i].ObjTip)],0,_x-x1-sx ,_y-y1-sy,
                            sx*3,sx*3,1,1,false,false,false,clwhite4,fxBlend);
   ...
   with MyCanvas do
     Begin
      ...
      _w:=trunc(Fonts[1].TextWidth(language[58])/ 2);
       ...
       Fonts[1].TextOut(_str,_x-_w+Menus[MenuN].buttons[2].x*resolutionscaleX,
          (533-Menus[MenuN].buttons[1].y)*resolutionscaleY2,
          cRGB1(255, 255, 255, trunc(_alf2*255)));
       ...
       Fonts[1].TextOut(_str,_x-_w+Menus[MenuN].buttons[2].x*resolutionscaleX,
          (563-Menus[MenuN].buttons[1].y)*resolutionscaleY2,
          cRGB1(255, 255, 255, trunc(_alf2*255)));
...
//       Fonts[1].Scale:=ResolutionScaleY2*1ю2;
       ...
       if level=0 then
         _str:=language[128]
          ...
          _str:=language[39]+inttostr(Globalscore)+' cr';
       _w:=trunc(Fonts[1].TextWidth(_str));
 ...
 Begin
  ...
  {     Fonts[1].Scale:=ResolutionScaleY2*0.7;
     ...
     // _w:=35*resolutionscaleY-35*resolutionscaleY2;
      ...
      if curbutton=1 then
       FillRect(trunc(500*ResolutionScaleX),
        trunc(310*resolutionscaleY2),
        trunc(50*ResolutionScaleX),
        trunc((30)*resolutionscaleY2),
        crgb1(10,100,200,trunc(_alf/1.5)),fxBlend)
          else
      FillRect(trunc(500*ResolutionScaleX),
        trunc(310*resolutionscaleY2),
        trunc(50*ResolutionScaleX),
      ...
      if curbutton=2 then
      FillRect(trunc(1050*ResolutionScaleX),
        trunc(310*resolutionscaleY2),
        trunc(50*ResolutionScaleX),
        trunc((30)*resolutionscaleY2),
        crgb1(10,100,200,trunc(_alf/1.5)),fxBlend)
 ...
 Begin
   ...
   if not(paused) then
     ...
     if (CurrentScreenN>1)and(Globalticks<10)and(CurrentScreenN<10) then
     Begin
     DrawStretch(Images2.Image[inttostr(CurrentScreenN-1)],0,
      ...
      Begin
                       // cxc
       _str:=language[80+i+(CurrentScreenN-1)*4];
     ...
     Begin
      ...
      _th:=(Globalticks-10*i)/10;
       ...
       _str:=copy(language[80+(CurrentScreenN)*4+i],1,trunc(length(language[80+i+(CurrentScreenN)*4])*_th));
     ...
     End;
      ...
      if (CurrentScreenN>=9)and(Globalticks>10) then
       Begin
         menuready:=false;
         UnloadIntro;
         nextmenu:=10;
    ...
    begin
      ...
      if itogo>percento[5] then
       ...
       _str:=Language[23]+' '+inttostr(percento[6])+' ';
        ...
        Fonts[1].TextOut(_str,
        (VirtualW*ResolutionScaleX-_h)/2, 620 *ResolutionScaleY2,
        cRGB1(255, 255, 255,round(fade1)));

       _h2:=round(Fonts[1].TextWidth(_str));

        Fonts[2].TextOut('+',
        (VirtualW*ResolutionScaleX-_h)/2+_h2, 627 *ResolutionScaleY2,
        cRGB1(255, 255, 255,round(fade1)));

       _h2:=_h2+round(Fonts[2].TextWidth('+'));

       _str:=' '+inttostr(trunc(itogo))+' = '+inttostr(percento[6]+trunc(itogo))
          +' '+language[41];

       Fonts[1].TextOut(_str,
        (VirtualW*ResolutionScaleX-_h)/2+_h2, 620 *ResolutionScaleY2,
        cRGB1(255, 255, 255,round(fade1)));


      _h:=round(Fonts[1].TextWidth(Language[24]));
      Fonts[1].TextOut(Language[24],
...
begin
  ...
  Begin
    ...
    if Radar then
    Begin
      if Minimapobjcount<200 then inc(Minimapobjcount);
             ...
             sparkeff5(GunPos[i].X,GunPos[i].Y,10,currentweapon);
              ...
              with  TArmoSprite.Create(Engine) do
               begin
                    ...
                    armopower:=round(10*(2-diff[difficulty]));
                    if plasmup then
                    Begin
                      armopower:=round(24*(2-diff[difficulty]));    {УСИЛЕННЫЕ ПУЛИ}
                      //ArmoType:=aTrasser2;
                    End;
      ...
      case TTile(Sprite).lines[i].lineId of
        ...
        if VeloY<0 then
          ...
          Begin
             ...
             if (TTile(Sprite).tip=5)or(TTile(Sprite).tip=31)
...
End;
 ...
 Begin
   ...
   Begin
     ...
     sparkeff2(x+SizeXd2,y+SizeYd2,pfire);
     sparkeff2(x+SizeXd2,y+SizeYd2,pfire);
 ...
 Begin
   cursoroncapsule:=true;
   if RV<400 then TakenCapsule:=self;
 End;
...
Begin
  if not((TTile(Sprite).tip=19)and(tip>3)and(keeping=false)) then
   if TTile(Sprite).mylinecount>0 then
    for I := 0 to TTile(Sprite).mylineCount - 1 do
      case TTile(Sprite).lines[i].lineId of
 ...
 Begin
    if (abs(x-gamcurx)<50)and (abs(y-gamcury)<50)then
      Begin
        cursoronbox:=true;
        cursoroncapsule:=true;
        if (RV<500)and(keepitm=false) then
          TakeBox:=self;
      End;
